...
bool should_stagger_image_set(bool compressed)
{
    ...
    // Temporarily bypass the staggered upload path for AMD while investigating
    // Windows driver-specific texture/material corruption.
    return !compressed && on_main_thread() && !gGLManager.mIsIntel && !gGLManager.mIsAMD;
#endif
}
